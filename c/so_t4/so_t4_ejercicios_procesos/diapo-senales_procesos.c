...
int main() {
	...
	printf("MAIN - PID: %d\n", pid);
	printf("MAIN - SIGNAL ()\n");
	// CON ESTO (signal) LE ESTOY DICIENDO QUE CUANDO RECIBA LA SEÑAL SIGUSR1 (por defecto termina el proceso, como ctrl+c)
	// EJECUTE LA FUNCIÓN MANEJADOR
	signal(SIGUSR1, manejador); 
	printf("MAIN - Siguiente instrucción es el FORK. Creo un proceso hijo.\n");
	pid = fork();
	printf("MAIN - Tras el fork, PID del proceso hijo es ahora: %d\n", pid);
	...
	if ( pid == 0 ) { 
		...
		printf("MAIN - PADRE \n");
		printf("MAIN - PADRE PID: %d\n", pid);
		// CON ESTO (kill) VOY A ENVIAR UNA SEÑAL A UN PROCESO
		// EN ESTE CASO, VOY A ENVIAR LA SEÑAL SIGUSR1 (por defecto es terminar el proceso) AL PROCESO PID
		kill(pid, SIGUSR1);
		wait(NULL);
...
/* PROGRAM OUTPUT
	MAIN - PID: 0
	MAIN - SIGNAL ()
	MAIN - Siguiente instrucción es el FORK. Creo un proceso hijo.
	MAIN - Tras el fork, PID del proceso hijo es ahora: 65000
	MAIN - PADRE
	MAIN - PADRE PID: 65000
	Manejador - Entro en el manejador:
	Manejador - PID: 0
	Manejador -  IF PID == 0:
	Manejador - 	Hijo: Recibida señal 30
	Manejador - 	PID es: 64999
	Manejador - 	KILL
	Manejador - Entro en el manejador:
	Manejador - PID: 65000
	Manejador -  IF PID != 0:
