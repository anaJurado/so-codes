...
int pipe[2];  //Para almacenar los descriptores de la tubería
...
void manejador_hijo(int sig)
{
	...
	printf("Estamos en el MANEJADOR\n");
...
int main() {
	...
	*/
	...
	if (pid < 0){
		...
		close(pipe[0]); //Cierro el extremo de lectura de la tubería
		while(1);  //Bucle infinito. El hijo solo hará algo cuando le llegue una señal
	}
	else {  				//Proceso padre

	    close(pipe[1]); // cierro el extremo de escritura en el pipe
		...
		while(fgets(buf,1024,stdin) != NULL) {			
			...
			// Al comienzo del main indicamos que si llegaba la seña SIGUSR1 ejecutara la función manejador_hilo
			// Por tando de aquí pasamo a manejador hilo
			printf("Antes KILL\n");
			kill(pid, SIGUSR1);		
			printf("Después KILL\n");	
